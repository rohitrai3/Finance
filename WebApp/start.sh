...
# Extract MY_APP_ environment variables and build the sed expression
...
if [ ! -s "$sed_script" ]; then
    echo "No MY_APP_ environment variables found. Exiting with error."
    exit 1
fi

# sed All files
find /app/dist -type f -exec sed -i -f "$sed_script" '{}' +
